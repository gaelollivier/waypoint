...
- **Duplicate cleanup**: duplicate detection is tied to one selected scan. It prefers `full_hash` groups when available and falls back to sampled groups for browsing only. Destructive cleanup is allowed only for full-hash-backed groups from that selected scan; before unlinking, Waypoint recomputes fresh sampled hashes for the kept file and every file to delete and requires them to still match the selected scan snapshot.
- **Per-disk exclusion list scope**: the `excluded_paths` table affects duplicate detection only. Files at or under an excluded path are filtered from both the Phase 1 GROUP BY and the per-group member lookups, so an excluded copy never forms or joins a group. Scan, diff, and copy intentionally ignore this list — the file is still indexed and still copied, it just doesn't surface as a duplicate-detection candidate. Rationale: the user's backup intent and the user's "what counts as a meaningful duplicate" intent are different problems. A self-contained archive that re-includes its own support files is legitimately a duplicate set but the user doesn't want it in their cleanup queue every run; that's a detection-view concern, not a copy-correctness concern.
